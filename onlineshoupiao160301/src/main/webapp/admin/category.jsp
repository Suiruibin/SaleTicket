...
<section class="container-fluid">
  ...
  <div class="row">
    <aside class="col-sm-3 col-md-2 col-lg-2 sidebar">
      ...
      <ul class="nav nav-sidebar">
       <c:if test="${manageuser.mark==1}">
      	 <li class="active"><a href="/getCategoryType.do?curPage=1">栏目</a></li>
      </c:if>
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
          ...
          <h1 class="page-header">添加</h1>
            ...
            <div class="form-group">
              ...
              <c:if test="${manageuser.change==1 }">
            	<button class="btn btn-primary" type="submit" name="submit" onclick="fn1()">添加新栏目</button>
             </c:if>
          ...
          <script type="text/javascript">
          			function fn1()
          			{
          				...
          				var describe=$("#category-describe").val();
          			  	...
          			  	$.post(
          					"/AdminAddCategoryServlet",
          					{"name":name,"alias":alias,"keywords":keywords,"describe":describe},
          					function(data)
          					{
          						alert(data);
                                windows.location.href="/getCategoryType.do?curPage="+${curPage}
        ...
        <div class="col-md-7">
          <h1 class="page-header">管理父栏目 <span class="badge">${totalCount }</span></h1>
          <div class="table-responsive">
            <table class="table table-striped table-hover">
 			...
 			<c:forEach items="${splitecategoryplist}" var="p" >
 				 <tr>
                	<td>${p.ID }</td>
                	<td>${p.tname }</td>
                	<td>${p.aname }</td>
                	<td>${p.keyword }</td>
                <td>
                  <%--<a  href="/operatesubtype.do?ID=${p.ID }&curPage=1">详情</a>--%>
               		 <c:if test="${manageuser.change==1 }">
                	 <a  href="/categorytypedetail.do?ID=${p.ID }">修改</a>
                	 <a  rel="${p.ID }">删除</a>
                	</c:if>
                </td>
                 </tr>
            ...
            <span class="prompt-text end " >
               ...
               &nbsp;   &nbsp;   &nbsp;   &nbsp;   &nbsp;
                  &nbsp;   &nbsp;   &nbsp;   &nbsp;   &nbsp;
                	...
                	<c:if test="${curPage!=1}">
					<a href="/getCategoryType.do?curPage=${curPage-1}" style="font-size:15px">上一页</a>&nbsp;&nbsp;
				</c:if>
				<c:if test="${curPage<totalPage }">
					<a href="/getCategoryType.do?curPage=${curPage+1}" style="font-size:15px">下一页</a>&nbsp;&nbsp;&nbsp;&nbsp;
				</c:if>
		
				<span style="font-size:14px">当前第${curPage}页</span>/
				<span style="font-size:14px" >共${totalPage}页</span>/
				<span style="font-size:14px">共有${totalCount}条记录</span>
	...
	$("#main table tbody tr td a").click(function(){
		...
		if (event.srcElement.outerText === "删除") 
		{
			if(window.confirm("此操作不可逆，是否确认？"))
			{
				$.ajax({
					type: "POST",
					url: "/deleteSuperType.do",
					data: "ID=" + id,
					cache: false, //不缓存此页面   
					success: function (data) {
						alert(data);
						 window.location.href="/getCategoryType.do?curPage="+${curPage};
					}
				});
			};
		};
