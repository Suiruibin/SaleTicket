    ...
    <div class="row">
      ...
      <ul class="nav nav-sidebar">
       <c:if test="${manageuser.mark==1}">
        ...
        <div class=" main setback" id="main">

                <div class="table-1">
                    <h1 class="page-header ">管理当前主栏目下的子栏目 <span class="badge">3</span></h1>
                    <br>
                    <table class="table table-striped table-hover">
						...
						<c:forEach items="${splitesubtypeplist}" var="p" >
 								 <tr>
                						<td>${p.ID }</td>
                						<td>${p.tname }</td>
                						<td>${p.otherName }</td>
                						<td>${p.keyword }</td>
               							 <td> 
               							  <c:if test="${manageuser.change==1 }">
                				 			 <a  href="/Semester_Demo/showsubtypedetail.do?ID=${p.ID }">修改</a> 
                				  			<a  rel="${p.ID }">删除</a>
                				  			</c:if>
              				 			</td>
                 				</tr>
            ...
            <c:if test="${subtypecurPage!=1}">
                <div class="table-2">
                    <a href="/Semester_Demo/operatesubtype.do?curPage=${subtypecurPage-1}&ID=${subtypeid}" style="color:#fff">上一页</a>&nbsp;&nbsp;
                </div>
            </c:if>
            <c:if test="${subtypecurPage<subtypetotalPage }">
                <div class="table-3">
                    <a href="/Semester_Demo/operatesubtype.do?curPage=${subtypecurPage+1}&ID=${subtypeid}" style="color:#fff">下一页</a>&nbsp;&nbsp;&nbsp;&nbsp;

                </div>
            </c:if>
                <div class="table-4">
                    <span style="font-size:14px">当前第${subtypecurPage }页</span>/
                    <span style="font-size:14px" >共${subtypetotalPage }页</span>/
                    <span style="font-size:14px">共有${subtypetotalCount }条记录</span>
                </div>
	...
	$("#main table tbody tr td a").click(function(){
		...
		if (event.srcElement.outerText === "删除") 
		{
			if(window.confirm("此操作不可逆，是否确认？"))
			{
				$.ajax({
					type: "POST",
					url: "/Semester_Demo/DeleteSubTypeServlet",
					data: "id=" + id,
					cache: false, //不缓存此页面   
					success: function (data) {
							alert(data);
					   window.location.href="/Semester_Demo/operatesubtype.do?curPage="+1+"&ID="+${sessionScope.subtypeid};
