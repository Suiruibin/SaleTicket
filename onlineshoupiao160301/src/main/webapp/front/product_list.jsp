...
//	    alert(li_id);
	    if(li_id!=null){
//            var element=document.getElementById(li_id).className="active";
			...
			<%--&lt;%&ndash;</ol>&ndash;%&gt;--%>
				...
				<div style="width: 1000px;margin-top: -30px;margin-left: 100px">
					...
					<ul class="nav nav-pills">
						...
						<c:forEach items="${townInfoList}" var="p" varStatus="status">
							<c:if test="${status.count<=15}">
								<li id="${p.id}">
									<a href="/ProductSubTypeListServlet?ID=${p.town}&currentPage=1&liId=${p.id}&cid=${cid}" onclick="outputCity()">${p.town}</a>
								</li>
							</c:if>
					...
					<ul class="nav nav-pills" style="display: none" id="moreInfo">
						<c:forEach items="${townInfoList}" var="p" varStatus="status">
							<c:if test="${status.count>15}">
								<li id="${p.id}"><a href="/ProductSubTypeListServlet?ID=${p.town}&currentPage=1&liId=${p.id}&moreInfoFLag=1&cid=${cid}">${p.town}</a></li>
