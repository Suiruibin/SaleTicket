...
//            response.sendRedirect(request.getContextPath()+"/front/login.jsp");
        ...
        order.setSend(0);
        order.setCancel(0);
        order.setDelivery(20);
        order.setConfirmOrder(0);
        order.setConfirmCancel(0);
        order.setRefuse(0);
...
//                int res=ticketService.addSeatLog(cartItem.getRoundID(),Integer.parseInt(s_row),Integer.parseInt(s_col),orderItem.getItemid());
        ...
        for(SeatLog p:seatLogList){
            int res=ticketService.addTempSeatLog(p.getRid(),p.getRow(),p.getCol(),p.getItemid(),p.getOid());
        }
