...
//		获得本用户的收藏或者未收藏
    ...
    public String ProductSubTypeListServlet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
        ...
        for(City p:tidList.getList()){
            ...
            if(ticket.getTypeBID()==Integer.parseInt(cid)){
...
//
        ...
        session.setAttribute("currentPage", currentPage);
        session.setAttribute("totalPage", ticketList.size()/12+1);
        session.setAttribute("totalCount",ticketList.size());
//
    ...
    @RequestMapping(value = "/SerachTicketResultServlet",method = POST)
    public String SerachTicketResultServlet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
        String bookName=request.getParameter("searchinput");



        Ticket ticket=ticketService.findTicketInfoByName(bookName);
//		浏览次数加1
        ...
        ticketService.addTicketInfoHit(ticket.getID(),ticket.getHit()+1);

        String  superTypename=ticketService.findSuperTypeName(ticket.getTypeBID());
        System.out.println(superTypename);


        request.setAttribute("superTypename", superTypename);
        request.setAttribute("subTypename", superTypename);
        request.setAttribute("product", ticket);
        request.setAttribute("currentPage", currentPage);
//		父栏目的ID
        request.setAttribute("cid", ticket.getTypeBID());

        request.setAttribute("isSearchResult", "1");
        ...
        String pids = Integer.toString(ticket.getID());
        Cookie[] cookies = request.getCookies();
        if(cookies!=null){
            for(Cookie cookie : cookies){
                if("pids".equals(cookie.getName())){
                    ...
                    LinkedList<String> list = new LinkedList<String>(asList);//[3,1,2]
                    //判断集合中是否存在当前pid
                    if(list.contains(Integer.toString(ticket.getID()))){
                        //包含当前查看商品的pid    //防止多次加入同一个商品
                        list.remove(Integer.toString(ticket.getID()));
                        list.addFirst(Integer.toString(ticket.getID()));
                    }else{
                        //不包含当前查看商品的pid 直接将该pid放到头上
                        list.addFirst(Integer.toString(ticket.getID()));
        ...
        return "redirect:/ProductInfoServlet?pid="+ticket.getID()+"&currentPage=1";
    }


    @RequestMapping(value = "/SearchTicketInfoServlet",method = POST)
    public void SearchTicketInfoServlet(HttpServletRequest request,HttpServletResponse response) throws IOException {
        String word=request.getParameter("word");

        List<String>  productlist=ticketService.findTicketByword(word);

//	 使用JSON的转换工具将服务器对象或者集合转成json格式的字符串
